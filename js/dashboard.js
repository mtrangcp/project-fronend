            ...
            <div   class="item-boards" style="${
              isImage
                ...
                <i class="fa-solid fa-star ${
                  item.is_starred ? "check-active" : ""
                }" onclick="markStarBoard(${index})"></i>
            </div>
        `;
  });
...
function openModalCreate() {
  ...
  listBackground.forEach((item) =>
    item.querySelector(".fa-solid")?.classList.remove("check-active")
  );
...
function openModalUpdate(index) {
  ...
  btnUpdate.onclick = function (event) {
    ...
    const createModal = bootstrap.Modal.getInstance(
      document.getElementById("scrollModalEdit")
    );
    if (createModal) {
      createModal.hide();
    }
...
function renderListBg(id) {
  ...
  listbg.innerHTML = "";
  listbg.innerHTML = arrBackground
    .map((item) => {
      return `
